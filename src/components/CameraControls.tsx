  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex justify-center space-x-1">
        <Button 
          className={`rounded-full px-3 py-1 text-xs ${frameColor === 'white' ? 'bg-white text-black' : 'bg-gray-200 text-black'}`}
          onClick={() => onFrameColorChange('white')}
        >
          White
        </Button>
        <Button 
          className={`rounded-full px-3 py-1 text-xs ${frameColor === 'green' ? 'bg-green-500 text-white' : 'bg-green-500/80 text-white'}`}
          onClick={() => onFrameColorChange('green')}
        >
          Green
        </Button>
        <Button 
          className="rounded-full px-3 py-1 text-xs bg-red-500 text-white"
        >
          More
        </Button>
      </div>
      ...
      <div className="flex justify-center space-x-4 bg-[#4b30ab] py-1.5 rounded-lg">
        ...
        <Button 
          variant="ghost" 
          className="text-white h-9 w-9 rounded-full border-2 border-white flex items-center justify-center"
      ...
      <div className="bg-[#1A1A1A] rounded-lg p-2">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="grid grid-cols-3 bg-[#1A1A1A] p-0.5">
            <TabsTrigger 
              value="frame-color" 
              className={`text-xs ${activeTab === 'frame-color' ? 'bg-[#333] text-white' : 'text-gray-400'}`}
            >
              Frame Color
            </TabsTrigger>
            <TabsTrigger 
              value="stickers" 
              className={`text-xs ${activeTab === 'stickers' ? 'bg-[#333] text-white' : 'text-gray-400'}`}
            >
              Stickers
            </TabsTrigger>
            <TabsTrigger 
              value="idol" 
              className={`text-xs ${activeTab === 'idol' ? 'bg-[#333] text-white' : 'text-gray-400'}`}
            >
              Idol
            </TabsTrigger>
          </TabsList>
